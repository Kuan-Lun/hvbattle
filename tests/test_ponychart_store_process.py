...
def _success(request: Mapping[str, object], root: Path) -> dict[str, object]:
    return process_module._success_as_json(  # noqa: SLF001
    ...
    *,
    ...
    def start(
        ...
        *,
        ...
        state = process_module._OwnedStoreProcess(  # noqa: SLF001
            request.request_id,
            "a" * 64,
            cast(OwnedProcess, process),
            None,
        )
        owner._owned[id(state)] = state  # noqa: SLF001
        return state
    ...
    async def connect(
        ...
        *,
        ...
        action(child, process_module._request_as_json(request), state)  # noqa: SLF001
        return parent
...
def test_default_config_crosses_ipc_without_import_time_path_resolution() -> None:
    ...
    encoded = process_module._config_as_json(config)  # noqa: SLF001
    assert encoded["root"] is None
    assert process_module._config_from_json(encoded) == config  # noqa: SLF001
...
def _send_success(
    ...
    channel.sendall(
        process_module._json_bytes(_success(request, Path(root)))
    )  # noqa: SLF001
    channel.close()
...
def _send_failure(
    ...
    del state
    channel.sendall(
        process_module._json_bytes(  # noqa: SLF001
            {
                "type": "failure",
...
def test_success_returns_descriptor_after_process_tree_is_reaped(
    ...
    async def exercise() -> None:
        ...
        assert owner._owned == {}  # noqa: SLF001
...
def test_supervisor_start_receives_one_shared_absolute_deadline(
    ...
    now = time.monotonic()
    request = process_module._StoreRequest(  # noqa: SLF001
        "request-id",
        process_module._StoreOperation.LOAD_OR_BOOTSTRAP,  # noqa: SLF001
        None,
        now + 10.0,
    ...
    with patch(
        ...
        state = owner._start(  # noqa: SLF001
            request,
...
def test_sync_receipt_decode_finishing_after_deadline_is_rejected() -> None:
    parent, child = socket.socketpair()
    original_decode = process_module._decode_json_payload  # noqa: SLF001
    ...
    async def exercise() -> None:
        ...
        try:
            ...
            with (
                ...
                await process_module._async_receive_json(  # noqa: SLF001
                    parent,
                    expires_at=time.monotonic() + 0.01,
                )
        finally:
...
def test_result_parsing_finishing_after_semantic_deadline_is_rejected(
    ...
    async def exercise() -> None:
        ...
        original_result = process_module._result_from_json  # noqa: SLF001
...
def test_store_cleanup_primitive_rejects_late_exit_proof(tmp_path: Path) -> None:
    async def exercise() -> None:
        ...
        with (
            ...
            await owner._settle_owned(  # noqa: SLF001
                state,
